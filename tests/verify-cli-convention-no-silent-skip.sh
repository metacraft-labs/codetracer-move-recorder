#!/usr/bin/env bash
# Verify that the codetracer-move-recorder CLI complies with
...
#     `CODETRACER_MOVE_RECORDER_DISABLED` are referenced in source so
...
BIN="${REPO_ROOT}/target/debug/codetracer-move-recorder"
if [[ ! -x "${BIN}" ]]; then
...
# ---------------------------------------------------------------------------
# `aptos-replay` subcommand --help
# ---------------------------------------------------------------------------

APTOS_REPLAY_HELP="$("${BIN}" aptos-replay --help)"

assert_absent "--format" "aptos-replay --help" "${APTOS_REPLAY_HELP}"
assert_absent "CODETRACER_FORMAT" "aptos-replay --help" "${APTOS_REPLAY_HELP}"
assert_present "--out-dir" "aptos-replay --help" "${APTOS_REPLAY_HELP}"
...
assert_present "codetracer-move-recorder" "--version output" "${VERSION_OUT}"

# ---------------------------------------------------------------------------
# Source-level reference for the env-var fallback
# ---------------------------------------------------------------------------

# The recorder must reference CODETRACER_MOVE_RECORDER_OUT_DIR in
# source (otherwise the env-var fallback either doesn't exist or has
# been silently removed).  We grep recursively under src/.
if ! grep -rqF "CODETRACER_MOVE_RECORDER_OUT_DIR" "${REPO_ROOT}/src"; then
  echo "FAIL: CODETRACER_MOVE_RECORDER_OUT_DIR must be referenced in src/" >&2
  exit 1
fi
echo "ok: CODETRACER_MOVE_RECORDER_OUT_DIR referenced in src/"

if ! grep -rqF "CODETRACER_MOVE_RECORDER_DISABLED" "${REPO_ROOT}/src"; then
  echo "FAIL: CODETRACER_MOVE_RECORDER_DISABLED must be referenced in src/" >&2
  exit 1
fi
echo "ok: CODETRACER_MOVE_RECORDER_DISABLED referenced in src/"
